    ...
    methods: {    
      ...
      async getHistory () {
        ...
        for (var team_id in this.teamHistories.team_totals) {
          ...
          this.teamDefChart['plot'].styles.push({'background-color': team_total.color});
        }
      ...
      async getHistoryByWeek (refresh = true) {       
        if (this.selectedSeason > 0) {
          if(this.selectedWeek > 0) {           
            if (this.historicalStats[this.selectedSeason][this.selectedWeek].players.length > 0) {
              this.selectedHistory = this.historicalStats[this.selectedSeason][this.selectedWeek].players;
              this.showHistoryTable = true;
            } else {
              let weekData = await this.NFLData.getJakesByWeek(this.selectedSeason, this.selectedWeek);
              this.historicalStats[this.selectedSeason][this.selectedWeek].players = weekData.players;
